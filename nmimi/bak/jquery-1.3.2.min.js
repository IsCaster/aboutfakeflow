...
(function()
{
  ...
  (function()
  {
    ...
    }; var I = F.selectors = 
    {
      ...
      {
        ID:  / #(( ? : [w u00c0 - uFFFF_ - ] | .) + ) / , CLASS:  / .(( ? : [w u00c0 - uFFFF_ - ] | .) + ) / , NAME:  / [name = ['"]*((?:[\w\u00c0-\uFFFF_-]|\\.)+)['"] * ] / , ATTR:  / [s * (( ? : [w u00c0 - uFFFF_ - ] | .) + )s * ( ? : (S ?  = )s * (['"] * )(. *  ? )3 | )s * ] / , TAG:  /  ^ (( ? : [w u00c0 - uFFFF * _ - ] | .) + ) / , CHILD:  / : (only | nth | last | first) - child( ? : ((even | odd | [dn +  - ] * ))) ?  / , POS:  / : (nth | eq | gt | lt | first | last | even | odd)( ? : ((d * ))) ? ( ?  = [ ^  - ] | $) / , PSEUDO:  / : (( ? : [w u00c0 - uFFFF_ - ] | .) + )( ? : ((['"]*)((?:\([^\)]+\)|[^\2\(\)]*)+)\2\))?/},attrMap:{" class ":" className ","for ":" htmlFor "},attrHandle:{href:function(T){return T.getAttribute(" href ")}},relative:{" + ":function(aa,T,Z){var X=typeof T===" string ",ab=X&&!/\W/.test(T),Y=X&&!ab;if (ab && !Z)
        {
          T = T.toUpperCase()
        }
      ...
      , "~": function(W, U, Y)
      {
        var V = L++, T = S; if (typeof U ==  = " string " && !U.match( /  \
          W / ))
        {
          var X = U = Y ? U : U.toUpperCase(); T = P
        }
      ...
      , CHILD: function(T)
      {
        if (T[1] == " nth ")
        {
          var U =  / ( -  ? )( \
            d * )n(( ? :  \
             +  |  - ) ?  \
            d * ) / .exec(T[2] == " even " && " 2n " || T[2] == " odd " && " 2n + 1 " || ! /  \
            D / .test(T[2]) && " 0n + " + T[2] || T[2]); T[2] = (U[1] + (U[2] || 1)) - 0; T[3] = U[3] - 0
        }
        T[0] = L++; return T
      }
  ...
  _load: o.fn.load, load: function(G, J, K)
  {
    ...
    var F = this; o.ajax(
    {
      url: G, type: H, dataType: " html ", data: J, complete: function(M, L)
      {
        ...
        {
          F.html(E ? o(" < div /  > ").append(M.responseText.replace( /  < script(. |  \
            s) *  ?  \
             / script >  / g, "")).find(E): M.responseText)
        }
        if (K)
        {
...
); var r = e(); o.extend(
{
  ...
  , ajax: function(M)
  {
    ...
    , o.ajaxSettings, M)); var W, F =  /=  \
       ? (& | $) / g, R, V, G = M.type.toUpperCase(); if (M.data && M.processData && typeof M.data !=  = "string")
    {
    ...
    {
      var E = e(); var U = M.url.replace( / ( \
         ?  |  & )_ = . *  ? (& | $) / , "$1_=" + E+"$2"); M.url = U + ((U == M.url) ? (M.url.match( /  \
         ?  / ) ? "&" : "?") + "_=" + E: "")
    }
    if (M.data && G == "GET")
    {
    ...
    var Q =  /  ^ ( \
      w + : ) ?  \
       /  \
       / ([ ^  \
       /  ? #] + ) / .exec(M.url); if (M.dataType == "script" && G == "GET" && Q && (Q[1] && Q[1] != location.protocol || Q[2] != location.host))
    {
